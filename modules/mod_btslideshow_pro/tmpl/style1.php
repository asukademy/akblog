...
$titleCSS = '#mod_btslideshow_pro_' . $moduleID . ' .label_skitter p{' .
        ($params->get('desc_font') ? '   font-family: ' . $params->get('desc_font') . ';' : '') .
        ($params->get('desc_size') ? '   font-size: ' . $params->get('desc_size') . 'px;' : '') .
        ($params->get('desc_color') ? '   color: ' . $params->get('desc_color') . ';' : '') .
        '}';
$document->addStyleDeclaration($titleCSS);
...
$document->addStyleDeclaration('
    #mod_btslideshow_pro_' . $moduleID . '.box_skitter .next_button{
        bottom: ' . $buttonBottom . 'px; right: 10px;
    }
    #mod_btslideshow_pro_' . $moduleID . '.box_skitter .prev_button{
        bottom: ' . $buttonBottom . 'px; right: 50px;
    }
    '
...
foreach ($items as $item) {
    if (!empty($item->link)) {
				if ($item->target != 'window') {
                        ...
                        $linkAttr = 'href="#"  ="window.open(\'' . $item->link . '\' , \'\', \'type=fullscreen, scrollbars=yes,toolbar=no,menubar=no,status=no\' ); return false;"';
                }
                $img = '<a '.$linkAttr.'>
...
//      $options[] = 'height_label: "' . $params->get('caption_height', 250) . 'px"';
...
$options = implode(",\n", $options);
//$out = sprintf("jQuery('#mod_btslideshow_pro_" . $module->id . "').skitter({%s});", $options);
?>
    </ul>
</div>
<div style="clear: both;"></div>
